...
HAL_StatusTypeDef rs485_interface_send(Txbuffer,TxbufferLength)
{
	rs485_interface_enableTxMode();
	HAL_StatusTypeDef transmissionStatus = HAL_UART_Transmit_DMA(&huart3, Txbuffer, TxbufferLength);
    rs485_interface_enableRxMode();
    return transmissionStatus;
}

HAL_StatusTypeDef rs485_interface_receive(Rxbuffer,RxbufferLength)
{
  rs485_interce_enableRxMode();
